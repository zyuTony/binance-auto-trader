...
if os.path.exists(strat_csv_file):
    strat_df = pd.read_csv(strat_csv_file)
    if strat_df.empty:
...
for index, row in monitored_pairs_df.iterrows():
   ...
   for _, order_row in orders_df.iterrows():
      ...
      if {symbol_Y, symbol_X} == order_pair_set and order_row['pair_trade_status'] in {"OPEN", "UPPER_STOPPED", "LOWER_STOPPED"}:
         already_traded = True
         print(f"ALREADY TRADED with status: {order_row['pair_trade_status']}.")
         break
   if already_traded:
      continue
   ...
   if len(Y_minute_data) != len(X_minute_data):
      print(f"Length mismatch between Y_minute_data ({len(Y_minute_data)}) and X_minute_data ({len(X_minute_data)})")
      continue  

   if len(Y_daily_data) != len(X_daily_data):
      print(f"Length mismatch between Y_daily_data ({len(Y_daily_data)}) and X_daily_data ({len(X_daily_data)})")
      continue
   
   # 2. calculate spread.
   minute_spread = calculate_spread(Y_minute_data, X_minute_data, ols_coeff, ols_constant)
   daily_spread_range = calculate_bollinger_bands(calculate_spread(Y_daily_data, X_daily_data, ols_coeff, ols_constant), BB_BAND_WINDOW, BB_SIGNAL_STD_MULT, BB_STOPLOSS_STD_MULT).drop(columns=['spread'])
   ...
   latest_strat = pd.DataFrame([{'date': latest_min['date'], 
                                 ...
                                 'symbol_X':symbol_X,
                                 'strategy': strat}])
   strat_df = pd.concat([strat_df, latest_strat], ignore_index=True)
   strat_df.to_csv(strat_csv_file, index=False)
   ...
   if latest_strat['strategy'].iloc[-1] != 'stand_by':
      ...
      curr_price_Y = round_step_size(Y_minute_data['close'].iloc[-1], 0.00001)
      curr_price_X = round_step_size(X_minute_data['close'].iloc[-1], 0.00001)

      # set tick size based on crypto price
      y_tick_size, x_tick_size = get_tick_size(curr_price_Y, curr_price_X)

      amt_Y = round_step_size(TOTAL_USDT_PER_TRADE/(curr_price_X*ols_coeff + curr_price_Y), y_tick_size)
